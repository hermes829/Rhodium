...
yData = merge(yData, wbData[,c(4:11,17,ncol(wbData))], by='cyear', all.x=T, all.y=F)
wbData$cyear = paste0(wbData$ccode, wbData$year-1)
yData = merge(yData, wbData[,c(4:11,17,ncol(wbData))], by='cyear', all.x=T, all.y=F, suffixes=c("_l0",""))
wbData$cyear = paste0(wbData$ccode, wbData$year+1)
