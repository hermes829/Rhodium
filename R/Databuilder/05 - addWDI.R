...
wbData=WDI(country='all', indicator=c("BX.KLT.DINV.CD.WD", "NY.GDP.PCAP.KD", "NY.GDP.PCAP.KD.ZG", "NY.GDP.MKTP.CD", "NY.GDP.MKTP.KD", "NY.GDP.MKTP.KD.ZG"), start=1988, end=2010, extra=T)
...
yData = merge(yData, wbData[,c(4:9,15,ncol(wbData))], by='cyear', all.x=T, all.y=F, suffixes=c("_l0",""))
wbData$cyear = paste0(wbData$ccode, wbData$year+1)
yData = merge(yData, wbData[,c(4:9,15,ncol(wbData))], by='cyear', all.x=T, all.y=F, suffixes=c("_l1","_p1"))
