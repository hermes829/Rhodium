...
% Note to Ben: Again if you could add anything to why The World Alamanac data is the right choice that would be great.
...
\begin{figure}
	\centering
	\resizebox{.8\textwidth}{!}{\input{distGdp}}
...
% Note to Ben: Does empirical model sound weird?
...
Aggregating each of the PRIO conflict descriptive measures in this way is to help account for those countries in which conflicts are especially intense, cover large areas, have been long-lasting, and multiple conflicts are occurring. Given the findings in the extant literature, our expectation is that each of these measures should have a negative effect on economic growth. Though we also expect that the substantive implications of any one of these measures will matter less than the adverse effect to economic growth by the spatial proximity of conflict to urban centers.
...
\begin{align*}
	...
	& \;+\; \beta_{4}(Ln(Conflict \; Area)_{i,t-1}) \;+\; \beta_{5}(Upper \; Income_{i,t}) \\	
...
% Note from Ben: I think we should leave the rmse plot out because we cant hope to actually predict gdp growth with this model setup. But we've done plenty of other important things with checking that our parameter estimates are somewhat substantive and stable, so I'm happy. That's why I've commented it out for now. If you want to put it back in feel free. If you do though, you do have to say something about the RMSE still being pretty damn high.
...
\begin{figure}
	...
	\begin{tabular}{cc}
		\subfloat[SubFigure 1][Capital City]{
			\resizebox{.7\textwidth}{!}{\input{mCapSimPlot}}
		\label{fig:capSim}} \\
		\subfloat[SubFigure 2][Any City]{
			\resizebox{.7\textwidth}{!}{\input{mCitySimPlot}}
		\label{fig:citySim}}
	\end{tabular}
